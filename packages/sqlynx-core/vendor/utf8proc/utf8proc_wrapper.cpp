...
size_t Utf8Proc::RenderWidth(std::string_view sv, size_t pos) {
	...
	auto len = sv.size();
    int sz;
    auto codepoint = sqlynx::utf8::utf8proc_codepoint(s + pos, sz);
    auto properties = sqlynx::utf8::utf8proc_get_property(codepoint);
...
size_t Utf8Proc::RenderWidth(const std::string &str) {
	...
	while(pos < str.size()) {
		int sz;
		auto codepoint = sqlynx::utf8::utf8proc_codepoint(str.c_str() + pos, sz);
		auto properties = sqlynx::utf8::utf8proc_get_property(codepoint);
