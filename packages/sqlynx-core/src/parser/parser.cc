...
namespace sqlynx::parser {

void sqlynx::parser::ParserBase::error(const location_type& loc, const std::string& message) {
    ctx.AddError(loc, message);
}

template <typename Base> static void destroy(std::string_view msg, sqlynx::parser::Parser::basic_symbol<Base>& yysym) {
    // See yy_destroy_
}
