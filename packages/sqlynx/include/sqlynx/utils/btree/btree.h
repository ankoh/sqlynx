...
struct btree_can_extract_key<_Pair, Key, std::pair<First, Second>>
    : std::conditional<std::is_same<typename std::remove_const<First>::type, Key>::value, btree_extract_key_first_tag,
                       btree_extract_key_fail_tag>::type {};
...
template <> struct btree_key_compare_to_adapter<std::less<std::string>> : public btree_key_compare_to_tag {
    btree_key_compare_to_adapter() {}
...
template <> struct btree_key_compare_to_adapter<std::greater<std::string>> : public btree_key_compare_to_tag {
    btree_key_compare_to_adapter() {}
...
// compare-to functor.
template <typename Key, typename Compare, bool HaveCompareTo> struct btree_key_comparer {
    btree_key_comparer() {}
...
template <typename Key, typename Compare> struct btree_key_comparer<Key, Compare, true> {
    btree_key_comparer() {}
    btree_key_comparer(Compare c) : comp(c) {}
...
struct btree_common_params {
    ...
    typedef std::conditional_t<btree_is_key_compare_to<Compare>::value, Compare, btree_key_compare_to_adapter<Compare>>
        key_compare;
...
// compare.
template <typename Key, typename Compare> struct btree_upper_bound_adapter : public Compare {
...
template <typename Key, typename CompareTo> struct btree_upper_bound_compare_to_adapter : public CompareTo {
...
template <typename K, typename N, typename Compare> struct btree_linear_search_plain_compare {
...
template <typename K, typename N, typename CompareTo> struct btree_linear_search_compare_to {
...
template <typename K, typename N, typename Compare> struct btree_binary_search_plain_compare {
    static int lower_bound(const K& k, const N& n, Compare comp) {
...
template <typename K, typename N, typename CompareTo> struct btree_binary_search_compare_to {
    static int lower_bound(const K& k, const N& n, CompareTo /*comp*/) {
...
template <typename Params> class btree_node {
   ...
   public:
    ...
    const key_type& key(int i) const { return params_type::get_key(fields_.values[i]); }
    ...
    template <typename Compare> int lower_bound(const key_type& k, const Compare& comp) const {
        return search_type::lower_bound(k, *this, comp);
    }
    // Returns the position of the first value whose key is greater than k.
    template <typename Compare> int upper_bound(const key_type& k, const Compare& comp) const {
    ...
    template <typename... Args> void insert_value(int i, Args&&... args);
   ...
   private:
    ...
    template <typename... Args> void construct_value(value_type* v, Args&&... args) {
...
template <typename Node, typename Reference, typename Pointer> class btree_iterator {
   public:
...
template <typename Params> class btree : public Params::key_compare {
   ...
   public:
    ...
    const_iterator cend() const { return const_iterator(rightmost(), rightmost() ? rightmost()->count() : 0); }
    ...
    template <typename... Args> std::pair<iterator, bool> emplace_unique_key_args(const key_type& key, Args&&... args);
    ...
    // Inserts a value into the btree only if it does not already exist. The
    // boolean return value indicates whether insertion succeeded or failed.
    template <typename... Args> std::pair<iterator, bool> emplace_unique(Args&&... args) {
        value_type v(std::forward<Args>(args)...);
        return emplace_unique_key_args(params_type::get_key(v), std::move(v));
    }

    template <class P> std::pair<iterator, bool> emplace_unique_extract_key(P&& x, btree_extract_key_fail_tag) {
        value_type v(std::forward<P>(x));
        return emplace_unique_key_args(params_type::get_key(v), std::move(v));
    }

    template <class P> std::pair<iterator, bool> emplace_unique_extract_key(P&& x, btree_extract_key_self_tag) {
        return emplace_unique_key_args(x, std::forward<P>(x));
    }

    template <class P> std::pair<iterator, bool> emplace_unique_extract_key(P&& x, btree_extract_key_first_tag) {
    ...
    template <typename P> std::pair<iterator, bool> emplace_hint_unique(const_iterator hint, P&& x) {
    ...
    // Inserts a value into the btree only if it does not already exist. The
    // boolean return value indicates whether insertion succeeded or failed.
    template <typename... Args> iterator emplace_hint_unique(const_iterator hint, Args&&... args) {
        value_type v(std::forward<Args>(args)...);
    ...
    template <typename... Args> iterator emplace_multi_key_args(const key_type& key, Args&&... args);
    ...
    // Inserts a value into the btree.
    template <typename... Args> iterator emplace_hint_multi(const_iterator hint, Args&&... args) {
        value_type v(std::forward<Args>(args)...);
        return emplace_hint_multi_key_args(hint, params_type::get_key(v), std::move(v));
    }

    iterator insert_multi(const value_type& v) { return emplace_multi_key_args(params_type::get_key(v), v); }
    iterator insert_multi(value_type&& v) { return emplace_multi_key_args(params_type::get_key(v), std::move(v)); }
   ...
   private:
    ...
    const node_type* rightmost() const { return (!root() || root()->is_leaf()) ? root() : root()->rightmost(); }
    node_type*& __rightmost() { return root()->__rightmost(); }

    // The leftmost node is stored as the parent of the root node.
    node_type* leftmost() { return root() ? root()->get_parent() : nullptr; }
    const node_type* leftmost() const { return root() ? root()->get_parent() : nullptr; }

    // The size of the tree is stored in the root node.
    size_type& __size() { return root()->__size(); }

    // Allocator routines.
    internal_allocator_type& __internal_allocator() noexcept { return *static_cast<internal_allocator_type*>(&root_); }
    const internal_allocator_type& internal_allocator() const noexcept {
        return *static_cast<const internal_allocator_type*>(&root_);
    }

    allocator_type allocator() const noexcept { return allocator_type(internal_allocator()); }
    ...
    iterator internal_end(const_iterator iter) { return iter.node ? iterator(iter.node, iter.position) : end(); }
    ...
    template <typename... Args> iterator internal_emplace(const_iterator hint, Args&&... args);
    ...
    template <typename IterType> std::pair<IterType, int> internal_locate(const key_type& key, IterType iter) const;
    ...
    template <typename IterType> IterType internal_lower_bound(const key_type& key, IterType iter) const;

    // Internal routine which implements upper_bound().
    template <typename IterType> IterType internal_upper_bound(const key_type& key, IterType iter) const;

    // Internal routine which implements find_unique().
    template <typename IterType> IterType internal_find_unique(const key_type& key, IterType iter) const;

    // Internal routine which implements find_multi().
    template <typename IterType> IterType internal_find_multi(const key_type& key, IterType iter) const;
   ...
   private:
    ...
    static std::conditional_t<
        std::conditional_t<is_key_compare_to::value, std::is_same<R, int>, std::is_same<R, bool>>::value, big_, small_>
    ...
    COMPILE_ASSERT((kNodeValues < (1 << (8 * sizeof(typename base_fields::field_type)))), target_node_size_too_large);
...
////
// btree_node methods
template <typename P> template <typename... Args> inline void btree_node<P>::insert_value(int i, Args&&... args) {
    auto cnt = count();
...
template <typename P> void btree_node<P>::rebalance_right_to_left(btree_node* src, int to_move) {
    auto cnt = count();
...
template <typename P> void btree_node<P>::rebalance_left_to_right(btree_node* dst, int to_move) {
    auto cnt = count();
...
template <typename P> void btree_node<P>::split(btree_node* dst, int insert_position) {
    auto cnt = count();
    auto dst_cnt = dst->count();
...
template <typename N, typename R, typename P> void btree_iterator<N, R, P>::increment_by(int count) {
    while (count > 0) {
        if (node->is_leaf()) {
...
template <typename P>
template <typename... Args>
std::pair<typename btree<P>::iterator, bool> btree<P>::emplace_unique_key_args(const key_type& key, Args&&... args) {
    if (empty()) {
...
template <typename P> typename btree<P>::iterator btree<P>::erase(const_iterator iter) {
...
template <typename P> int btree<P>::erase(const_iterator begin, const_iterator end) {
...
template <typename P> bool btree<P>::try_merge_or_rebalance(const_iterator& iter) {
...
template <typename P> void btree<P>::internal_dump(std::ostream& os, const node_type* node, int level) const {
    for (int i = 0; i < node->count(); ++i) {
        if (!node->is_leaf()) {
...
template <typename Tree> class btree_container {
   ...
   public:
    ...
    // Lookup routines.
    iterator lower_bound(const key_type& key) { return __tree.lower_bound(key); }
    const_iterator lower_bound(const key_type& key) const { return __tree.lower_bound(key); }
    iterator upper_bound(const key_type& key) { return __tree.upper_bound(key); }
    const_iterator upper_bound(const key_type& key) const { return __tree.upper_bound(key); }
    std::pair<iterator, iterator> equal_range(const key_type& key) { return __tree.equal_range(key); }
    std::pair<const_iterator, const_iterator> equal_range(const key_type& key) const { return __tree.equal_range(key); }

    allocator_type get_allocator() const noexcept { return allocator_type(__tree.allocator()); }
...
template <typename Tree> class btree_unique_container : public btree_container<Tree> {
   ...
   public:
    ...
    const_iterator find(const key_type& key) const { return this->__tree.find_unique(key); }
    size_type count(const key_type& key) const { return this->__tree.count_unique(key); }
#if __cplusplus > 201703L
    bool contains(const key_type& key) const { return find(key) != this->end(); }
#endif

    // Insertion routines.
    std::pair<iterator, bool> insert(const value_type& x) { return this->__tree.insert_unique(x); }
    template <typename P> std::pair<iterator, bool> insert(P&& x) {
        return this->__tree.insert_unique(std::forward<P>(x));
    }
    std::pair<iterator, bool> insert(value_type&& x) { return this->__tree.insert_unique(std::move(x)); }
    iterator insert(const_iterator hint, const value_type& x) { return this->__tree.insert_unique(hint, x); }
    template <typename P> std::pair<iterator, bool> insert(const_iterator hint, P&& x) {
        return this->__tree.insert_unique(hint, std::forward<P>(x));
    }
    iterator insert(const_iterator hint, value_type&& x) { return this->__tree.insert_unique(hint, std::move(x)); }
    void insert(std::initializer_list<value_type> il) { insert(il.begin(), il.end()); }
    template <typename InputIterator> void insert(InputIterator f, InputIterator l) {
    ...
    void erase(const iterator& first, const iterator& last) { this->__tree.erase(first, last); }
};
...
template <typename Tree> class btree_multi_container : public btree_container<Tree> {
   ...
   public:
    ...
    const_iterator find(const key_type& key) const { return this->__tree.find_multi(key); }
    size_type count(const key_type& key) const { return this->__tree.count_multi(key); }
#if __cplusplus > 201703L
    bool contains(const key_type& key) const { return find(key) != this->end(); }
#endif

    // Insertion routines.
    iterator insert(const value_type& x) { return this->__tree.insert_multi(x); }
    template <typename P> iterator insert(P&& x) { return this->__tree.insert_multi(std::forward<P>(x)); }
    iterator insert(value_type&& x) { return this->__tree.insert_multi(std::move(x)); }
    iterator insert(const_iterator hint, const value_type& x) { return this->__tree.insert_multi(hint, x); }
    template <typename P> iterator insert(const_iterator hint, P&& x) {
        return this->__tree.insert_multi(hint, std::forward<P>(x));
    }
    iterator insert(const_iterator hint, value_type&& x) { return this->__tree.insert_multi(hint, std::move(x)); }
    template <typename InputIterator> void insert(InputIterator f, InputIterator l) {
        for (const_iterator end = this->cend(); f != l; ++f) {
            insert(end, *f);
        }
