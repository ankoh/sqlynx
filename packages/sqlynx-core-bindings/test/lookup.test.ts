...
describe('Lookup', () => {
    ...
    describe('script table refs', () => {
        describe('lower bound', () => {
            ...
            it('single hit', () => {
                const script = packScript([
                    tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, 3), 0)
                ], [])
                const tmp = new sqlynx.proto.IndexedTableReference();
            ...
            it('skip 1', () => {
                const script = packScript([
                    tableRef(DB_ID, SCHEMA_ID - 1, sqlynx.ContextObjectID.create(42, 2), 0),
                    tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, 3), 0)
                ], [])
                const tmp = new sqlynx.proto.IndexedTableReference();
            ...
            it('100 different, last', () => {
                ...
                for (let i = 0; i < 100; ++i) {
                    tableRefs.push(tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, i), 0));
                }
                tableRefs.push(tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(43, 1), 0))
                const script = packScript(tableRefs, []);
                const tmp = new sqlynx.proto.IndexedTableReference();
                const iter = sqlynx.findScriptTableRefsLowerBound(script, tmp, 0, tableRefs.length, DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(43, 0));
                expect(iter).toEqual(100);
            });
            it('100 different, first', () => {
                const tableRefs: sqlynx.proto.IndexedTableReferenceT[] = [];
                tableRefs.push(tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(41, 1), 0))
                for (let i = 0; i < 100; ++i) {
                    tableRefs.push(tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, i), 0));
                }
                const script = packScript(tableRefs, []);
                const tmp = new sqlynx.proto.IndexedTableReference();
                const iter = sqlynx.findScriptTableRefsLowerBound(script, tmp, 0, tableRefs.length, DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(41, 0));
                expect(iter).toEqual(0);
            });
            it('100 different, mid', () => {
                ...
                tableRefs.push(tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(43, 1), 0))
                for (let i = 0; i < 50; ++i) {
                    tableRefs.push(tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(44, i), 0));
                }
                const script = packScript(tableRefs, []);
                const tmp = new sqlynx.proto.IndexedTableReference();
                const iter = sqlynx.findScriptTableRefsLowerBound(script, tmp, 0, tableRefs.length, DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(43, 0));
            ...
            it('100 equal, skip first', () => {
                const tableRefs: sqlynx.proto.IndexedTableReferenceT[] = [];
                tableRefs.push(tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(41, 2), 0))
                for (let i = 0; i < 100; ++i) {
                    tableRefs.push(tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, 3), i));
                }
                const script = packScript(tableRefs, []);
                const tmp = new sqlynx.proto.IndexedTableReference();
                const iter = sqlynx.findScriptTableRefsLowerBound(script, tmp, 0, tableRefs.length, DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(41, 3));
                expect(iter).toEqual(1);
            });
            it('100 equal, skip 100', () => {
                const tableRefs: sqlynx.proto.IndexedTableReferenceT[] = [];
                for (let i = 0; i < 100; ++i) {
                    tableRefs.push(tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(41, 2), i))
                }
                for (let i = 0; i < 100; ++i) {
                    tableRefs.push(tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, 3), i));
                }
                for (let i = 0; i < 100; ++i) {
                    tableRefs.push(tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(41, 4), i))
                }
                const script = packScript(tableRefs, []);
                const tmp = new sqlynx.proto.IndexedTableReference();
                const iter = sqlynx.findScriptTableRefsLowerBound(script, tmp, 0, tableRefs.length, DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(41, 3));
        ...
        describe('upper bound', () => {
            ...
            it('single miss', () => {
                ...
                const tmp = new sqlynx.proto.IndexedTableReference();
                const iter = sqlynx.findScriptTableRefsUpperBound(script, tmp, 0, 1, DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, 3));
                expect(iter).toEqual(1);
            });
            it('skip 1', () => {
                const script = packScript([
                    tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, 2), 0),
                    tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, 3), 0)
                ], [])
                const tmp = new sqlynx.proto.IndexedTableReference();
                const iter = sqlynx.findScriptTableRefsUpperBound(script, tmp, 0, 2, DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, 2));
                expect(iter).toEqual(1);
            });
            it('100 equal, skip 100', () => {
                const tableRefs: sqlynx.proto.IndexedTableReferenceT[] = [];
                for (let i = 0; i < 100; ++i) {
                    tableRefs.push(tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, 2), i))
                }
                for (let i = 0; i < 100; ++i) {
                    tableRefs.push(tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, 3), i));
                }
                for (let i = 0; i < 100; ++i) {
                    tableRefs.push(tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, 4), i))
                }
                const script = packScript(tableRefs, []);
                const tmp = new sqlynx.proto.IndexedTableReference();
                const iter = sqlynx.findScriptTableRefsUpperBound(script, tmp, 0, tableRefs.length, DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, 3));
        ...
        describe('equal range', () => {
            ...
            it('single hit', () => {
                ...
                const iter = sqlynx.findScriptTableRefsEqualRange(script, DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, 3));
            ...
            it('single miss, lower', () => {
                ...
                const iter = sqlynx.findScriptTableRefsEqualRange(script, DB_ID, SCHEMA_ID - 1, sqlynx.ContextObjectID.create(42, 3));
                expect(iter).toEqual([0, 0]);
            });
            ...
            it('100 equal, skip 200, trailing 400', () => {
                ...
                for (let i = 0; i < 400; ++i) {
                    tableRefs.push(tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(43, 4), i))
                }
                const script = packScript(tableRefs, []);
                const iter = sqlynx.findScriptTableRefsEqualRange(script, DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, 3));
                expect(iter).toEqual([200, 300]);
            ...
            it('100 equal, skip 200, trailing 400, schema prefix', () => {
                const tableRefs: sqlynx.proto.IndexedTableReferenceT[] = [];
                for (let i = 0; i < 200; ++i) {
                    tableRefs.push(tableRef(DB_ID, SCHEMA_ID - 1, sqlynx.ContextObjectID.create(41, 2), i))
                }
                for (let i = 0; i < 100; ++i) {
                    tableRefs.push(tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, 3), i));
                }
                for (let i = 0; i < 400; ++i) {
                    tableRefs.push(tableRef(DB_ID, SCHEMA_ID + 1, sqlynx.ContextObjectID.create(43, 4), i))
                }
                const script = packScript(tableRefs, []);
            ...
            it('100 equal, skip 200, trailing 400, db prefix', () => {
                const tableRefs: sqlynx.proto.IndexedTableReferenceT[] = [];
                for (let i = 0; i < 200; ++i) {
                    tableRefs.push(tableRef(DB_ID - 1, 0, sqlynx.ContextObjectID.create(41, 2), i))
                }
                for (let i = 0; i < 100; ++i) {
                    tableRefs.push(tableRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, 3), i));
                }
                for (let i = 0; i < 400; ++i) {
                    tableRefs.push(tableRef(DB_ID + 1, 0, sqlynx.ContextObjectID.create(43, 4), i))
                }
                const script = packScript(tableRefs, []);
    ...
    describe('script column refs', () => {
        describe('equal range', () => {
            ...
            it('single hit', () => {
                const script = packScript([], [
                    columnRef(DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, 3), 0, 0)
                ]);
                const iter = sqlynx.findScriptColumnRefsEqualRange(script, DB_ID, SCHEMA_ID, sqlynx.ContextObjectID.create(42, 3), 0);
                expect(iter).toEqual([0, 1]);
            });
            it('single miss, lower', () => {
                const script = packScript([], [
                ...
                const iter = sqlynx.findScriptColumnRefsEqualRange(script, DB_ID, SCHEMA_ID - 1, sqlynx.ContextObjectID.create(42, 3), 0);
                expect(iter).toEqual([0, 0]);
            });
            it('single miss, upper', () => {
                const script = packScript([], [
    ...
    describe('catalog table ids', () => {
        ...
        it('single hit', () => {
            const catalog = packCatalog([], [], [
                tableEntryById(sqlynx.ContextObjectID.create(42, 43), 0)
            ])
            const iter = sqlynx.findCatalogTableById(catalog, sqlynx.ContextObjectID.create(42, 43));
            expect(iter).toEqual(0);
        });
        it('single miss', () => {
            const catalog = packCatalog([], [], [
            ...
            const iter = sqlynx.findCatalogTableById(catalog, sqlynx.ContextObjectID.create(42, 44));
            expect(iter).toEqual(null);
        });
    });
});
