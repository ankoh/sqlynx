...
static void generate_analyzer_dumps(const std::filesystem::path& source_dir) {
    auto dump_dir = source_dir / "dumps" / "completion";
    for (auto& p : std::filesystem::directory_iterator(dump_dir)) {
        ...
        for (auto test : root.children()) {
            ...
            // Encode a program
            // CompletionDumpTest::EncodeScript(test, *main_analyzed.first, external_analyzed.first.get());
            // XXX
        }
...
int main(int argc, char* argv[]) {
    gflags::SetUsageMessage("Usage: ./dump_completion --source_dir <dir>");
