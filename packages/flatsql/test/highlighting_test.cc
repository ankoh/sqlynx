...
TEST(HighlightingTest, InsertChars) {
    ...
    auto add_char = [&](char c, std::vector<uint32_t> offsets, std::vector<proto::HighlightingTokenType> types) {
        ...
        match_tokens(result->data_ptr, offsets, types);
    ...
    add_char(' ', {0, 6}, {Token::KEYWORD, Token::NONE});
    add_char('1', {0, 6, 7, 8}, {Token::KEYWORD, Token::NONE, Token::LITERAL_INTEGER, Token::NONE});
