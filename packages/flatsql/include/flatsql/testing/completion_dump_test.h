...
struct CompletionDumpTest {
    /// Printer test name
    struct TestPrinter {
        std::string operator()(const ::testing::TestParamInfo<const CompletionDumpTest*>& info) const {
    ...
    /// Get the grammar tests
    static std::vector<const CompletionDumpTest*> GetTests(std::string_view filename);
};

extern void operator<<(std::ostream& out, const CompletionDumpTest& p);
