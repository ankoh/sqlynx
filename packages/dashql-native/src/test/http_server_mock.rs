...
mod test {
    ...
    async fn test_http_service_mock() -> Result<()> {
        ...
        let handler = tokio::spawn(async move {
            ...
            headers.insert("dashql-test-header", HeaderValue::from_str("foo").unwrap());
        ...
        let result = reqwest::get(host)
            .await?;
        assert!(result.headers().contains_key("dashql-test-header"));
        assert_eq!(result.headers().get("dashql-test-header").unwrap(), "foo");
        let body = result
            .bytes()
            .await?;
