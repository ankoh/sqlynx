...
#include "flatsql/text/string_utils.h"
...
struct Rope {
    ...
    struct LeafNode {
        ...
        /// Get the raw buffer
        auto GetRawBuffer() noexcept { return std::span<std::byte>{buffer, LEAF_NODE_CAPACITY}; }
        /// Get buffer content as string
        auto GetString() noexcept { return std::string_view{reinterpret_cast<char*>(buffer), GetSize()}; }
        ...
        void Insert(size_t ofs, std::span<std::byte> data) noexcept {
            ...
            assert(isUTF8CodepointBoundary(GetRawBuffer(), ofs));
        ...
        void RemoveRange(size_t start_byte_idx, size_t end_byte_idx) noexcept {
            ...
            assert(isUTF8CodepointBoundary(GetRawBuffer(), start_byte_idx));
            assert(isUTF8CodepointBoundary(GetRawBuffer(), end_byte_idx));
