...
const DASHQL_PORTAL_ROOT_ID = '__dashqlPortalRoot__'
...
/// with id __dashqlPortalRoot__, allow that element to serve as the default portal root.
...
function ensureDefaultPortal() {
    ...
    if (!existingDefaultContainer || !document.body.contains(existingDefaultContainer)) {
        let defaultContainer = document.getElementById(DASHQL_PORTAL_ROOT_ID)
        if (!(defaultContainer instanceof Element)) {
            defaultContainer = document.createElement('div')
            defaultContainer.setAttribute('id', DASHQL_PORTAL_ROOT_ID)
