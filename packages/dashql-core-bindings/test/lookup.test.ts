...
function tableRef(db: number, schema: number, table: bigint, refId: number) {
    return new dashql.buffers.IndexedTableReferenceT(db, schema, table, refId);
}
function columnRef(db: number, schema: number, table: bigint, column: number, refId: number) {
    return new dashql.buffers.IndexedColumnReferenceT(db, schema, table, column, refId);
}
function databaseEntryById(db: number, index: number) {
    return new dashql.buffers.IndexedFlatDatabaseEntryT(db, index);
}
function schemaEntryById(schema: number, index: number) {
    return new dashql.buffers.IndexedFlatSchemaEntryT(schema, index);
}
function tableEntryById(table: bigint, index: number) {
    return new dashql.buffers.IndexedFlatTableEntryT(table, index);
}

function packScript(tableRefs: dashql.buffers.IndexedTableReferenceT[], columnRefs: dashql.buffers.IndexedColumnReferenceT[]) {
    const script = new dashql.buffers.AnalyzedScriptT();
    ...
    return dashql.buffers.AnalyzedScript.getRootAsAnalyzedScript(buffer);
}
function packCatalog(databasesById: dashql.buffers.IndexedFlatDatabaseEntryT[], schemasById: dashql.buffers.IndexedFlatSchemaEntryT[], tablesById: dashql.buffers.IndexedFlatTableEntryT[]) {
    const script = new dashql.buffers.FlatCatalogT();
    ...
    builder.finish(ofs);
    const buffer = builder.dataBuffer();
    return dashql.buffers.FlatCatalog.getRootAsFlatCatalog(buffer);
}
...
describe('Lookup', () => {
    ...
    describe('script table refs', () => {
        describe('lower bound', () => {
            it('empty', () => {
                ...
                const tmp = new dashql.buffers.IndexedTableReference();
            ...
            it('100 different, last', () => {
                const tableRefs: dashql.buffers.IndexedTableReferenceT[] = [];
                for (let i = 0; i < 100; ++i) {
    ...
    describe('catalog database ids', () => {
        ...
        it('skip 100, 200 trailing', () => {
            const databases: dashql.buffers.IndexedFlatDatabaseEntryT[] = [];
            let nextDb = 222;
            let nextIdx = 0;
            for (let i = 0; i < 100; ++i) {
    ...
    describe('catalog schema ids', () => {
        ...
        it('skip 100, 200 trailing', () => {
            const schemas: dashql.buffers.IndexedFlatSchemaEntryT[] = [];
            let nextDb = 2222;
            let nextIdx = 0;
            for (let i = 0; i < 100; ++i) {
