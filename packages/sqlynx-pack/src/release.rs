...
// Find first file that roughly matches `SQLynx_0.1.0_universal.dmg`
fn find_dmg_in(dir: &PathBuf) -> Option<PathBuf> {
    ...
    for dir_entry_access in dir_entries {
        if let Ok(dir_entry) = dir_entry_access {
            ...
            if file_name_str.starts_with("SQLynx_") && file_name_str.ends_with("_universal.dmg") {
...
impl Release {
    pub async fn build(args: ReleaseArgs) -> anyhow::Result<Self> {
        ...
        let macos_universal_update_dir = args
            ...
            .join("bundle")
            .join("macos");
        let macos_universal_update_app = macos_universal_update_dir.join("SQLynx.app.tar.gz");
        if macos_universal_update_app.is_file() {
            let remote_path = format!("{}/macos/SQLynx.app.tar.gz", remote_paths.release_directory);
            let remote_url = format!("{}/{}", &args.remote_base_url, &remote_path);
            let sig_path = macos_universal_update_dir.join("SQLynx.app.tar.gz.sig");
